...
class Gaussian_Shading_chacha:
    def __init__(self, ch_factor, hw_factor, fpr, user_number, device='cuda', channel=4, height=64):
        ...
        self.key = None
        self.watermark = None
        self.latentlength = channel * self.height * self.height
    ...
    def stream_key_encrypt(self, sd):
        self.key = b'\x9f\xa3\x8d\x94\xb2\xf1\x06\x93\x1f\xe5\xbb\x0d\x8d\x60\xf0\x62\x56\x79\xb5\xfa\xe3\x44\x84\x1a\xd9\xed\x2f\xf7\x23\xe9\x32\x7d'

        self.nonce = b'\xa7\x56\x4f\x2c\x3b\x01\xf3\xdc\x90\x9f\x84\x72'
    ...
    def truncSampling(self, message):
        ...
        z = torch.from_numpy(z).reshape(1, self.channel, self.height, self.height).half()
        return z.to(self.device)

    def create_watermark_and_return_w(self):
        try:
            self.watermark = torch.from_numpy(np.load('/research/d1/gds/ztli/WatermarkAttacker/watermark.npy')).to(self.device)
        except:
            # fix the seed as 100
            torch.manual_seed(100)
            self.watermark = torch.randint(0, 2, [1, self.channel // self.ch, self.height // self.hw, self.height // self.hw]).to(self.device)
            # save the watermark tensor to the watermark tensor into npy file
            np.save('/research/d1/gds/ztli/WatermarkAttacker/watermark.npy', self.watermark.cpu().numpy())
    ...
    def stream_key_decrypt(self, reversed_m):
        ...
        sd_tensor = torch.from_numpy(sd_bit).reshape(1, self.channel, self.height, self.height).to(torch.uint8)
        return sd_tensor.to(self.device)

    def diffusion_inverse(self,watermark_r):
        ch_stride = self.channel // self.ch
...
class Gaussian_Shading:
    def __init__(self, ch_factor, hw_factor, fpr, user_number, device='cuda', channel=4, height=64):
        ...
        self.key = None
        self.watermark = None
        self.latentlength = self.channel * self.height * self.height
        self.marklength = self.latentlength//(self.ch * self.hw * self.hw)
    ...
    def create_watermark_and_return_w(self):
        self.key = torch.randint(0, 2, [1, self.channel, self.height, self.height]).to(self.device)
        self.watermark = torch.randint(0, 2, [1, self.channel // self.ch, self.height // self.hw, self.height // self.hw]).to(self.device)
        sd = self.watermark.repeat(1,self.ch,self.hw,self.hw)
