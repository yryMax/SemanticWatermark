...
from diffusion_utils import ModifiedStableDiffusion3Pipeline, ModifiedPixArtAlphaPipeline
...
def main(args):
    ...
    synthesizers = [
        ModifiedPixArtAlphaPipeline.from_pretrained("PixArt-alpha/PixArt-XL-2-512x512", torch_dtype=torch.float16),

    ]
    for i in range(len(synthesizers)):
        synthesizers[i] = synthesizers[i].to(device)
    detector = ModifiedPixArtAlphaPipeline.from_pretrained("PixArt-alpha/PixArt-XL-2-512x512", torch_dtype=torch.float16, scheduler=detector_scheduler)
    ...
    # Class for watermark
    if args.chacha:
        watermark = Gaussian_Shading_chacha(args.channel_copy, args.hw_copy, args.fpr, args.user_number, channel=4, height=64)
    else:
        watermark = Gaussian_Shading(args.channel_copy, args.hw_copy, args.fpr, args.user_number, channel=4, height=64)
