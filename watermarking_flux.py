...
from diffusion_utils import ModifiedStableDiffusion3Pipeline, ModifiedPixArtAlphaPipeline, ModifiedFluxPipeline, RFInversionFluxPipeline
...
def main(args):
    ...
    detector_scheduler = DDIMInverseScheduler.from_pretrained("black-forest-labs/FLUX.1-schnell", subfolder='scheduler')

    synthesizers = [
        ModifiedFluxPipeline.from_pretrained("black-forest-labs/FLUX.1-schnell", torch_dtype=torch.bfloat16)


    ]
    for i in range(len(synthesizers)):
        synthesizers[i] = synthesizers[i].to(device)
    detector = RFInversionFluxPipeline.from_pretrained("black-forest-labs/FLUX.1-schnell", torch_dtype=torch.bfloat16)
    ...
    for i in tqdm(range(0, len(prompts))):
        ...
        for synthesizer in synthesizers:
            ...
            # Reverse image
            reversed_latents_w, _, _ = detector.invert(
                image_w_distortion,
                height=args.image_length,
                width=args.image_length,
            ...
            # Accuracy metric
            acc_metric = watermark.eval_watermark(reversed_latents_w.reshape(1, 16, 64, 64))
