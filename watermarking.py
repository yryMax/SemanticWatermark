...
def main(args):
    ...
    schedulers = {
        "PixArtAlpha": DPMSolverMultistepScheduler.from_pretrained("PixArt-alpha/PixArt-XL-2-512x512", subfolder='scheduler'),
        "StableDiffusion3": DPMSolverMultistepScheduler.from_pretrained("stabilityai/stable-diffusion-3-medium-diffusers", subfolder='scheduler'),
        "StableDiffusion2": DPMSolverMultistepScheduler.from_pretrained("stabilityai/stable-diffusion-2-1-base", subfolder='scheduler')
    }

    detector_scheduler = DDIMScheduler.from_pretrained(args.model_path, subfolder='scheduler')

    synthesizers = [
        PixArtAlphaPipeline.from_pretrained("PixArt-alpha/PixArt-XL-2-512x512", torch_dtype=torch.float16),
        ...
        StableDiffusionPipeline.from_pretrained("stabilityai/stable-diffusion-2-1-base",
                                                 scheduler=schedulers["StableDiffusion2"], torch_dtype=torch.float16),
    ]
    for i in range(len(synthesizers)):
        synthesizers[i] = synthesizers[i].to(device)
    detector = InversableStableDiffusionPipeline.from_pretrained(
        args.model_path,
        scheduler=detector_scheduler,  # Using a default scheduler for the detector
        torch_dtype=torch.float16,
        revision='fp16',
    ...
    tester_prompt = ''
    text_embeddings = detector.get_text_embedding(tester_prompt)
    ...
    for i in tqdm(range(0, len(prompts))):
        ...
        for synthesizer, scheduler_name in zip(synthesizers, schedulers.keys()):
            synthesizer_pipeline = synthesizer
            current_scheduler = schedulers[scheduler_name]
            ...
            reversed_latents_w = detector.forward_diffusion(
            ...
            outputs = synthesizer_pipeline(
                ...
                height=args.image_length,
                width=args.image_length,
                latents=reversed_latents_w.to(device),
            )
            image_w = outputs.images[0]
            output_image_path = os.path.join(args.output_path,
                                             f'{synthesizer_pipeline.__class__.__name__}_image_{i}_watermarked.png')
